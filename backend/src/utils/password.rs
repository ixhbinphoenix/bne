...
pub enum PasswordError {
    ...
    #[error("Password doesn't contain enough Special characters. Minimum special characters: {}", PASS_MIN_SYMBOLS)]
    NotEnoughSymbols,
...
pub fn valid_password(password: &str) -> Result<(), PasswordError> {
    if password.len() < PASS_MIN_LENGTH { return Err(PasswordError::TooShort); }
    if password.len() > PASS_MAX_LENGTH { return Err(PasswordError::TooLong); }
    if contains_number(password, SPECIAL_SYMBOLS) < PASS_MIN_SYMBOLS { return Err(PasswordError::NotEnoughSymbols); }
    if contains_number(password, DIGITS) < PASS_MIN_DIGITS { return Err(PasswordError::NotEnoughDigits); }
    if contains_number(password, LETTERS) < PASS_MIN_LETTERS { return Err(PasswordError::NotEnoughLetters); }
    Ok(())
}
